 ...
 */
export async function indexHistory(limit = 50) {
  ensureTable();
  const db = getDb();
  ...
  const insert = db.prepare(
    'INSERT OR REPLACE INTO rag_vectors (source, chunk_id, content, vector) VALUES (?, ?, ?, ?)'
  );
  const deleteOld = db.prepare('DELETE FROM rag_vectors WHERE source = ? AND chunk_id = ?');
  ...
  try {
    for (let i = 0; i < pairs.length; i++) {
      ...
      deleteOld.run('history', chunkId);
      insert.run('history', chunkId, texts[i], JSON.stringify(vectors[i]));
      count++;
    }
    db.exec('COMMIT');
  } catch (e) {
...
export async function indexPromptFile(filePath) {
  ...
  // 清除该文件已有的分块
  const basename = filePath.replace(/^.*[\\/]/, '').replace(/\.[^.]+$/, '');
  db.prepare('DELETE FROM rag_vectors WHERE source = ? AND chunk_id LIKE ?').run(source, `${basename}-%`);
  ...
  try {
    for (let i = 0; i < chunks.length; i++) {
      if (!vectors[i]) continue;
      insert.run(source, `${basename}-${i}`, chunks[i], JSON.stringify(vectors[i]));
      count++;
    }
    db.exec('COMMIT');
  } catch (e) {
 ...
 */
export async function search(query, topK = 5, threshold = 0.3) {
  ensureTable();
  const db = getDb();
  ...
  const rows = db.prepare('SELECT id, source, content, vector FROM rag_vectors').all();
...
export function clearIndex() {
  ensureTable();
  const db = getDb();
  const info = db.prepare('DELETE FROM rag_vectors').run();
  return info.changes;
}
